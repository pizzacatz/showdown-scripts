...
|--------|---------|-------------|
| [Ghost Clicker](ghost-clicker/) | 4.11 | Replaces Showdown's Random Battle default with **Reg M-B Bo3** (once per page load, never re-enforced) and adds native-styled **Reg M-B** / **Reg M-B (Bo3)** buttons below the format selector that select the format and start searching in one click. |
