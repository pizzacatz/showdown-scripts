...
12. **Toolbar lives inside `.battle-controls`** (v1.1–1.4). Everything in a battle room is absolutely positioned, so a normal-flow sibling renders behind the arena. The client rewrites the controls every turn; the observer re-injects the toolbar and buttons re-render from stored state. Placement: bottom-left below the move buttons with a 64px gap and separator.
13. **Instant battle end** (v1.5–1.6). On any battle ending — own forfeit, opponent forfeit, win, loss — the script clicks the client's native "Skip to end" button (once per battle) so the end screen and replay jobs arrive without waiting for the closing animations.
14. **Upload popup auto-dismissed** (v1.6–1.7). Once the replay link is captured from the "Your replay has been uploaded!" popup, its Close button is clicked automatically (`replay.autoDismissUploadPopup` toggle). Upload confirmation matches the link **only inside `.ps-popup`** — replay links elsewhere on the page must not confirm the upload.
15. **Upload and download remain parallel** through all of the above; skip-to-end and popup dismissal don't sequence the jobs, they only make both finish sooner.
